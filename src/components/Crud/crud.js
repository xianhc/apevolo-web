...
function CRUD(options) {
  ...
  const methods = {
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        ...
        initData(crud.url, crud.getQueryParams()).then(data => {
          ...
          crud.pagination.totalElements = data.totalElements
    ...
    doExport() {
      ...
      download(crud.url.replace('query', 'download'), crud.getQueryParams()).then(result => {
        downloadFile(result, crud.title + '数据')
    ...
    getQueryParams: function() {
      ...
      return {
        pageIndex: crud.pagination.pageIndex,
        pageSize: crud.pagination.pageSize,
        sortFields: crud.sortFields,
        ...crud.query,
        ...crud.params
      }
    },
    ...
    dleChangePage(size) {
      if (crud.data.length === size && crud.pagination.pageIndex !== 1) {
        crud.pagination.pageIndex -= 1
      }
    },
    // 选择改变
...
function pagination() {
  return {
    data() {
      return {
        crud: this.crud,
        pagination: this.crud.pagination
      }
    },
    beforeCreate() {
      this.crud = lookupCrud(this)
